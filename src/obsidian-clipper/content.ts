...
import * as highlighter from "./utils/highlighter.ts";
import { generalSettings, loadSettings } from "./utils/storage-utils.ts";
import Defuddle from "defuddle";
import { getDomain } from "./utils/string-utils.ts";
...
(function () {
  ...
  if (globalThis.hasOwnProperty("obsidianHighlighterInitialized")) {
    return; // Exit if already initialized
  }

  // Mark as initialized
  globalThis.obsidianHighlighterInitialized = true;
  ...
  browser.runtime.onMessage.addListener(
    (request: any, sender, sendResponse) => {
  ...
  browser.runtime.onMessage.addListener(
    (
      ...
      if (request.action === "getPageContent") {
        ...
        browser.runtime.sendMessage({ action: "getHighlighterMode" }).then(
          sendResponse,
        );
        return true;
        ...
        if (request.targetElementInfo) {
          ...
          if (!elementToHighlight) {
            const elements = Array.from(
              document.getElementsByTagName(mediaType),
            );
          ...
          if (elementToHighlight) {
            ...
            console.warn(
              "Could not find element to highlight. Info:",
              request.targetElementInfo,
            );
          }
        }
        updateHasHighlights();
  ...
  globalThis.addEventListener("load", updateHasHighlights);
