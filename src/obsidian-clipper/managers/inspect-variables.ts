...
export async function showVariables(isUpdate: boolean = false) {
  ...
  if (currentTemplate && Object.keys(currentVariables).length > 0) {
    ...
    if (!isUpdate) {
      variablesPanel.innerHTML = `
				<div class="variables-header">
  					<div class="variables-header-title">
						...
						<span class="close-panel clickable-icon" aria-label="${
        getMessage("close")
      }">
							<i data-lucide="x"></i>
						</span>
					</div>
					<input type="text" id="variables-search" placeholder="${
        getMessage("searchVariables")
      }">
				</div>
      ...
      // Search variables
      const searchInput = variablesPanel.querySelector(
        "#variables-search",
      ) as HTMLInputElement;
      if (searchInput) {
        searchInput.addEventListener(
          "input",
          debounce(handleVariableSearch, 300),
        );
      }

      const closePanel = variablesPanel.querySelector(
        ".close-panel",
      ) as HTMLElement;
      if (closePanel) {
      ...
      const variableList = variablesPanel.querySelector(
        ".variable-list",
      ) as HTMLElement;
      if (variableList) {
...
function handleVariableSearch() {
  ...
  variableItems.forEach((item: Element) => {
    ...
    if (currentSearchTerm.length < 2) {
      ...
      keyText.includes(currentSearchTerm) ||
      valueText.includes(currentSearchTerm)
    ) {
      htmlItem.style.display = "flex";
