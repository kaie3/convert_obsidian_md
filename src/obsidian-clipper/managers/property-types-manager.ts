...
function ensureTagsProperty(): void {
  const tagsProperty = generalSettings.propertyTypes.find((pt) =>
    pt.name === "tags"
  );
  if (!tagsProperty) {
...
export function updatePropertyTypesList(): void {
  ...
  const deleteUnusedButton = document.getElementById(
    "delete-unused-properties-btn",
  );
  ...
  sortedPropertyTypes.forEach((propertyType) => {
    ...
    const listItem = createPropertyTypeListItem(
      propertyType,
      propertyUsageCounts[propertyType.name] || 0,
      isUsed,
    );
  ...
  const deleteUnusedButtonContainer = deleteUnusedButton.closest(
    ".setting-item",
  );
  if (deleteUnusedButtonContainer instanceof HTMLElement) {
    deleteUnusedButtonContainer.style.display = hasUnusedProperties
      ? "flex"
      : "none";
  }
...
function createPropertyTypeListItem(
  ...
  propertySelectedDiv.appendChild(
    createElementWithHTML("i", "", {
      "data-lucide": getPropertyTypeIcon(propertyType.type),
    }),
  );
  ...
  if (usageCount === 0 && propertyType.name !== "tags") {
    ...
    removeBtn.addEventListener(
      "click",
      () => removePropertyType(propertyType.name),
    );
  } else {
    const removeBtn = createElementWithClass(
      "button",
  ...
  if (propertyType.name !== "tags") {
    select.addEventListener("change", function () {
      ...
      updatePropertyType(propertyType.name, this.value, defaultValueInput.value)
        .then(updatePropertyTypesList);
    });

    defaultValueInput.addEventListener("change", function () {
      updatePropertyType(propertyType.name, select.value, this.value).then(
        updatePropertyTypesList,
      );
    });
  } else {
...
async function importTypesFromJson(jsonContent: string): Promise<void> {
  try {
    ...
    if (
      content && typeof content === "object" && "types" in content &&
      typeof content.types === "object"
    ) {
      const newTypes = Object.entries(content.types).map(([name, type]) => {
        if (typeof type !== "string") {
          console.warn(
            `Invalid type for property "${name}". Using 'text' as default.`,
          );
      ...
      throw new Error(
        'Invalid types.json format: "types" property not found or is not an object',
      );
    }
  } catch (error) {
    console.error("Error parsing types.json:", error);
    throw new Error(
      `Error importing types.json: ${
...
async function mergePropertyTypes(newTypes: PropertyType[]): Promise<void> {
  ...
  for (const newType of newTypes) {
    ...
    if (newType.name === "tags") {
      ...
      const existingType = generalSettings.propertyTypes.find((pt) =>
        pt.name === newType.name
      );
      if (existingType) {
        console.log(
          `Existing type found for ${newType.name}: ${existingType.type}`,
        );
        if (existingType.type !== newType.type) {
          const useNewType = await resolveConflict(
            newType.name,
            "type",
            existingType.type,
            newType.type,
          ...
          if (useNewType) {
            console.log(
              `Updating existing type: ${newType.name} to ${newType.type}`,
            );
            ...
            console.log(
              `Keeping existing type: ${newType.name} as ${existingType.type}`,
            );
          }
        } else {
...
export async function addPropertyType(
  ...
  console.log(
    `addPropertyType called with: name=${name}, type=${type}, defaultValue=${defaultValue}`,
  );
  const existingPropertyType = generalSettings.propertyTypes.find((pt) =>
    pt.name === name
  );
  ...
  console.log(
    ...
    JSON.stringify(generalSettings.propertyTypes, null, 2),
  );
}
...
export async function updatePropertyType(
  ...
  if (index !== -1) {
    ...
    if (
      newDefaultValue !== undefined && newDefaultValue !== null &&
      newDefaultValue !== ""
    ) {
...
export async function removePropertyType(name: string): Promise<void> {
  generalSettings.propertyTypes = generalSettings.propertyTypes.filter((p) =>
    p.name !== name
  );
  await saveSettings();
  updatePropertyTypesList();
...
async function deleteUnusedProperties(): Promise<void> {
  ...
  const unusedProperties = generalSettings.propertyTypes.filter((pt) =>
    !usedProperties.has(pt.name) && pt.name !== "tags"
  );
  ...
  const confirmMessage =
    `Are you sure you want to remove ${unusedProperties.length} unused properties?`;
  if (confirm(confirmMessage)) {
    generalSettings.propertyTypes = generalSettings.propertyTypes.filter((pt) =>
      usedProperties.has(pt.name) || pt.name === "tags"
    );
    await saveSettings();
    updatePropertyTypesList();
