...
const STORE_URLS = {
  ...
  edge:
    "https://microsoftedge.microsoft.com/addons/detail/obsidian-web-clipper/eigdjhmgnaaeaonimdklocfekkaanfme",
};
...
export async function setShortcutInstructions() {
  ...
  if (shortcutInstructionsElement) {
    ...
    switch (browser) {
      case "chrome":
        instructions = getMessage("shortcutInstructionsChrome", [
          "<strong>chrome://extensions/shortcuts</strong>",
        ]);
        break;
      case "brave":
        instructions = getMessage("shortcutInstructionsBrave", [
          "<strong>brave://extensions/shortcuts</strong>",
        ]);
        break;
      case "firefox":
        instructions = getMessage("shortcutInstructionsFirefox", [
          "<strong>about:addons</strong>",
        ]);
        break;
      case "edge":
        instructions = getMessage("shortcutInstructionsEdge", [
          "<strong>edge://extensions/shortcuts</strong>",
        ]);
        break;
      case "safari":
      case "mobile-safari":
    ...
    shortcutInstructionsElement.innerHTML =
      getMessage("shortcutInstructionsIntro") + " " + instructions;
  }
}
...
async function initializeVersionDisplay(): Promise<void> {
  ...
  if (
    currentBrowser !== "safari" && currentBrowser !== "mobile-safari" &&
...
export function initializeGeneralSettings(): void {
  loadSettings().then(async () => {
    ...
    if (
      rateExtensionSection && totalClips >= 20 && existingRatings.length === 0
    ) {
...
function saveSettingsFromForm(): void {
  ...
  const updatedSettings = {
    ...
    highlightBehavior: highlightBehaviorSelect?.value ??
      generalSettings.highlightBehavior,
  };
  ...
  if (browser === "mobile-safari") {
    ...
    getCommands().then((commands) => {
      commands.forEach((command) => {
        ...
        hotkeySpan.textContent = command.shortcut ||
...
export function resetDefaultTemplate(): void {
  ...
  const defaultIndex = currentTemplates.findIndex((t: Template) =>
    t.name === getMessage("defaultTemplateName")
  );
...
async function handleRating(rating: number) {
  ...
  if (rating >= 4) {
    ...
    globalThis.open(storeUrl, "_blank");
  } else {
