...
export function updateTemplateList(loadedTemplates?: Template[]): void {
  ...
  const validTemplates = templatesToUse.filter((
    ...
    template != null && typeof template === "object" && "id" in template &&
    "name" in template
  );
  ...
  validTemplates.forEach((template, index) => {
    ...
    const deleteBtn = createElementWithClass(
      "button",
      "delete-template-btn clickable-icon",
    );
...
export function showTemplateEditor(template: Template | null): void {
  ...
  if (!template) {
    ...
    setEditingTemplateIndex(
      templates.findIndex((t) => t.id === editingTemplate.id),
    );
  }
  ...
  if (templateEditorTitle) {
    templateEditorTitle.textContent = getMessage("editTemplate");
  }
  ...
  const noteNameFormat = document.getElementById(
    "note-name-format",
  ) as HTMLInputElement;
  if (noteNameFormat) {
  ...
  const noteContentFormat = document.getElementById(
    "note-content-format",
  ...
  const triggersTextarea = document.getElementById(
    "url-patterns",
  ) as HTMLTextAreaElement;
  if (triggersTextarea) {
    triggersTextarea.value = editingTemplate && editingTemplate.triggers
  ...
  if (!editingTemplate.id) {
    const templateNameField = document.getElementById(
      "template-name",
    ) as HTMLInputElement;
  ...
  const vaultSelect = document.getElementById(
    "template-vault",
  ) as HTMLSelectElement;
  if (vaultSelect) {
...
function updateBehaviorFields(): void {
  ...
  const noteNameFormatContainer = document.getElementById(
    "note-name-format-container",
  );
  const pathContainer = document.getElementById("path-name-container");
  const noteNameFormat = document.getElementById(
    "note-name-format",
  ) as HTMLInputElement;
  const behaviorWarningContainer = document.getElementById(
    "behavior-warning-container",
  );

  if (behaviorSelect) {
    const selectedBehavior = behaviorSelect.value;
    const isDailyNote = selectedBehavior === "append-daily" ||
      selectedBehavior === "prepend-daily";
    const needsWarning = selectedBehavior !== "create" &&
      selectedBehavior !== "overwrite";

    if (needsWarning) {
      if (behaviorWarningContainer) {
        behaviorWarningContainer.style.display = "flex";
      }
    } else {
      if (behaviorWarningContainer) {
        behaviorWarningContainer.style.display = "none";
      }
    }

    if (noteNameFormatContainer) {
      noteNameFormatContainer.style.display = isDailyNote ? "none" : "block";
    }
    if (pathContainer) {
...
export function addPropertyToEditor(
  name: string = "",
  value: string = "",
  id: string | null = null,
): HTMLElement {
  ...
  const propertySelectedDiv = createElementWithClass(
    "div",
    "property-selected",
  );
  const propertyType =
    generalSettings.propertyTypes.find((p) => p.name === name)?.type || "text";
  propertySelectedDiv.dataset.value = propertyType;
  propertySelectedDiv.appendChild(
    createElementWithHTML("i", "", {
      "data-lucide": getPropertyTypeIcon(propertyType),
    }),
  );
  ...
  ["text", "multitext", "number", "checkbox", "date", "datetime"].forEach(
    (optionValue) => {
      const option = document.createElement("option");
      option.value = optionValue;
      option.textContent = optionValue.charAt(0).toUpperCase() +
        optionValue.slice(1);
  ...
  const removeBtn = createElementWithClass(
    "button",
    "remove-property-btn clickable-icon",
  );
  ...
  if (select) {
    select.addEventListener("change", function () {
      ...
      updatePropertyType(currentName, this.value).then(() => {
        console.log(
          `Property type for ${currentName} updated to ${this.value}`,
        );
      }).catch((error) => {
        console.error(
          `Failed to update property type for ${currentName}:`,
          error,
        );
      });
  ...
  nameInput.addEventListener("input", function (this: HTMLInputElement) {
    const selectedType = generalSettings.propertyTypes.find((pt) =>
      pt.name === this.value
    );
    if (selectedType) {
      ...
      if (this.value.trim() !== "") {
        updatePropertyType(this.value, selectedType.type).then(() => {
          console.log(
            `Property type for ${this.value} updated to ${selectedType.type}`,
          );
        }).catch((error) => {
          console.error(
            `Failed to update property type for ${this.value}:`,
            error,
          );
        });
      }
...
function updateSelectedOption(
  ...
  const iconElement = createElementWithHTML("i", "", {
...
export function updateTemplateFromForm(): void {
  ...
  if (behaviorSelect) {
    template.behavior = behaviorSelect.value as Template["behavior"];
  }
  ...
  if (noteNameFormat) {
    if (!isDailyNote && noteNameFormat.value.trim() === "") {
      console.error(
        "Note name format is required for non-daily note behaviors",
      );
  ...
  const propertyElements = document.querySelectorAll(
    "#template-properties .property-editor",
  );
  template.properties = Array.from(propertyElements).map((prop) => {
    ...
    const valueInput = prop.querySelector(
      ".property-value",
    ) as HTMLInputElement;
    const typeSelect = prop.querySelector(
      ".property-select .property-selected",
    ) as HTMLElement;
    return {
      id: (prop as HTMLElement).dataset.id ||
        Date.now().toString() + Math.random().toString(36).slice(2, 11),
...
function clearTemplateEditor(): void {
  ...
  const templateProperties = document.getElementById("template-properties");
  if (templateEditorTitle) {
    templateEditorTitle.textContent = getMessage("newTemplate");
  }
...
function handleAddProperty(): void {
  ...
  if (templateProperties) {
    ...
    const nameInput = newPropertyDiv.querySelector(
      ".property-name",
    ) as HTMLInputElement;
    if (nameInput) {
      nameInput.focus();
