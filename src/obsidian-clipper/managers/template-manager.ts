import { Property, PropertyType, Template } from "../types/types";
...
export async function loadTemplates(): Promise<Template[]> {
  try {
    ...
    if (templateIds.length > 0) {
      ...
      templates = loadedTemplates.filter((t: Template | null): t is Template =>
        t !== null
      );
    }
...
async function prepareTemplateForSave(
  template: Template,
  ...
  if (compressedData.length > SIZE_WARNING_THRESHOLD) {
    return [
      chunks,
      `Warning: Template "${template.name}" is ${
        (compressedData.length / 1024).toFixed(2)
      }KB, which is approaching the storage limit.`,
    ];
  }
  return [chunks, null];
}
...
async function updateGlobalPropertyTypes(templates: Template[]): Promise<void> {
  const existingTypes = new Set(
    generalSettings.propertyTypes.map((p) => p.name),
  );
  ...
  templates.forEach((template) => {
    template.properties.forEach((property) => {
      if (!existingTypes.has(property.name)) {
        const defaultType = defaultTypes[property.name] ||
