...
const PROVIDERS_URL =
  "https://raw.githubusercontent.com/obsidianmd/obsidian-clipper/refs/heads/main/providers.json";
...
async function fetchPresetProviders(): Promise<Record<string, PresetProvider>> {
  ...
  try {
    ...
    for (const key in data) {
      if (
        key !== "version" && Object.prototype.hasOwnProperty.call(data, key)
      ) {
...
async function getLocalPresets(): Promise<
  Record<string, PresetProvider> | null
> {
  try {
    const data = await getLocalStorage(LOCAL_STORAGE_KEY) as
      | ProviderPresets
      | null;
    if (!data) return null;
...
async function shouldUpdatePresets(): Promise<boolean> {
  try {
    const localData = await getLocalStorage(LOCAL_STORAGE_KEY) as
      | ProviderPresets
      | null;
...
> {
  ...
  if (
    isFetching ||
    (lastErrorTime > 0 && now - lastErrorTime < PRESET_RETRY_DELAY)
  ) {
    ...
    debugLog(
      "Providers",
      "Returning fallback presets (local or previous cache)",
    );
  ...
  try {
    ...
    if (!needsUpdate) {
      ...
      debugLog(
        "Providers",
        "Local presets missing despite version match or failed check, fetching fresh.",
      );
    }
    ...
    debugLog(
      "Providers",
      "Fetch failed, returning fallback presets (local or previous cache)",
    );
    return localPresets || cachedPresets || {};
  } finally {
    isFetching = false;
...
export async function initializeInterpreterSettings(): Promise<void> {
  ...
  if (addProviderBtn) {
    addProviderBtn.addEventListener(
      "click",
      (event) => addProviderToList(event),
    );
  }
}
...
function initializeProviderList() {
  ...
  sortedProviders.forEach((provider, index) => {
    const originalIndex = generalSettings.providers.findIndex((p) =>
      p.id === provider.id
    );
...
function createProviderListItem(
  ...
  providerItem.innerHTML = `
    ...
    provider.name.toLowerCase().replace(/\s+/g, "-")
			...
			${
    hasNoKey
      ? `<span class="provider-no-key"><i data-lucide="alert-triangle"></i> <span class="mh">${
        getMessage("apiKeyMissing")
      }</span></span>`
      : ""
  }
		</div>
  ...
  if (editBtn) {
    editBtn.addEventListener("click", (e) => {
      ...
      if (providerId) {
        const providerIndex = generalSettings.providers.findIndex((p) =>
          p.id === providerId
        );
        if (providerIndex !== -1) {
...
function deleteProvider(index: number): void {
  ...
  const modelsUsingProvider = generalSettings.models.filter((m) =>
    m.providerId === providerToDelete.id
  );
...
async function showProviderModal(provider: Provider, index?: number) {
  ...
  if (form) {
    ...
    if (
      !apiKeyContainer || !apiKeyDescription || !nameContainer ||
      !presetSelect || !nameInput || !baseUrlInput || !apiKeyInput
    ) {
      console.error("Required provider modal elements not found");
      return;
    }

    presetSelect.innerHTML = `<option value="">${
      getMessage("custom")
    }</option>`;
    ...
    if (index !== undefined) {
      ...
      if (!currentPresetId) {
        const nameMatchingPreset = Object.entries(cachedPresetProviders || {})
    ...
    const updateVisibility = () => {
      ...
      if (selectedPreset) {
        ...
        const editingOriginalPreset = index !== undefined &&
        ...
        apiKeyContainer.style.display = selectedPreset.apiKeyRequired === false
          ? "none"
          : "block";

        if (
          selectedPreset.apiKeyRequired !== false && selectedPreset.apiKeyUrl
        ) {
          ...
          } <a href="${selectedPreset.apiKeyUrl}" target="_blank">${message}</a>`;
        } else {
  ...
  newConfirmBtn?.addEventListener("click", async () => {
    ...
    const presetId =
      (form.querySelector('[name="preset"]') as HTMLSelectElement).value;
    ...
    if (presetId && cachedPresetProviders && cachedPresetProviders[presetId]) {
      ...
      updatedProvider.baseUrl = baseUrl !== providerPresetBaseUrl
        ? baseUrl
...
export function initializeModelList() {
  ...
  sortedModels.forEach((model) => {
    const originalIndex = generalSettings.models.findIndex((m) =>
      m.id === model.id
    );
    if (originalIndex !== -1) {
...
function createModelListItem(model: ModelConfig, index: number): HTMLElement {
  ...
  const provider = generalSettings.providers.find((p) =>
    p.id === model.providerId
  );
  const providerName = provider?.name ||
    `<span class="model-provider-unknown"><i data-lucide="alert-triangle"></i> ${
      getMessage("unknownProvider")
    }</span>`;
  ...
  if (checkbox) {
    ...
    checkbox.addEventListener("change", () => {
      const modelIndex = generalSettings.models.findIndex((m) =>
        m.id === model.id
      );
      if (modelIndex !== -1) {
...
async function showModelModal(model: ModelConfig, index?: number) {
  ...
  if (form) {
    ...
    if (
      !modelIdDescriptionContainer || !modelSelectionContainer ||
      !modelSelectionRadios || !nameInput || !providerModelIdInput ||
      !providerSelect
    ) {
    ...
    const updateModelOptions = () => {
      ...
      if (provider && cachedPresetProviders) {
        ...
        if (presetProvider?.modelsList) {
          modelIdDescriptionContainer.innerHTML = `${
            getMessage("providerModelIdDescription")
          } <a href="${presetProvider.modelsList}" target="_blank">${
            getMessage("modelsListFor", provider.name)
          }</a>.`;
        }
        ...
        if (presetProvider?.popularModels?.length) {
          ...
          presetProvider.popularModels.forEach((popModel, idx) => {
            ...
            radio.innerHTML = `
							...
							<label for="${radioId}">
								${popModel.name}${
              popModel.recommended
                ? ` <span class="tag">${getMessage("recommended")}</span>`
                : ""
            }
							</label>
						`;
          ...
          const popularMatch = presetProvider.popularModels.some((pm) =>
            pm.id === model.providerModelId
          );
          if (
            index !== undefined && model.providerId === selectedProviderId &&
            !popularMatch
          ) {
            const otherInput = otherRadio.querySelector(
              "input",
            ) as HTMLInputElement;
            if (otherInput) otherInput.checked = true;
          } else if (index === undefined) {
            const recommended = presetProvider.popularModels.find((pm) =>
              pm.recommended
            );
            if (!recommended) {
              const otherInput = otherRadio.querySelector(
          ...
          modelSelectionRadios.addEventListener("change", (e) => {
            ...
            if (target.value === "other") {
              if (
                !(index !== undefined &&
                  model.providerId === selectedProviderId && !popularMatch &&
              ...
              const selectedPopModel = presetProvider.popularModels?.find((m) =>
                m.id === target.value
              );
              if (selectedPopModel) {
    ...
    newConfirmBtn.addEventListener("click", async () => {
      ...
      const modelSelectionRadio = form.querySelector(
        'input[name="model-selection"]:checked',
      ) as HTMLInputElement;
      ...
      if (
        !updatedModel.name || !updatedModel.providerId ||
