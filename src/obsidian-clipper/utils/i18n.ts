...
export function setDayjsLocale(locale: string): void {
  ...
  try {
    ...
    console.warn(
      `Failed to set dayjs locale for ${locale}, falling back to English`,
      error,
    );
    dayjs.locale("en");
  }
...
export function getMessage(
  ...
  try {
    ...
    const messages = require(
      `../_locales/${currentLanguage || "en"}/messages.json`,
    );
      ...
      if (currentLanguage !== "en") {
        ...
        if (enMessageObj) {
          ...
          if (enMessageObj.placeholders) {
            Object.entries(enMessageObj.placeholders).forEach(
              ([key, value]) => {
...
export async function getEffectiveLanguage(): Promise<
  { code: string; isRTL: boolean }
> {
  const currentLang = await getCurrentLanguage();
  const languageCode = currentLang && currentLang !== ""
    ? currentLang
