...
export function createHighlighterMenu() {
  ...
  menu.innerHTML = `
    ${
    highlightCount > 0
      ? `<button id="obsidian-clip-button" class="mod-cta">Clip highlights</button>`
      : '<span class="no-highlights">Select elements to highlight</span>'
  }
    ${
    highlightCount > 0
      ? `<button id="obsidian-clear-highlights">${highlightText} <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-trash-2"><path d="M3 6h18"/><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"/><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"/><line x1="10" x2="10" y1="11" y2="17"/><line x1="14" x2="14" y1="11" y2="17"/></svg></button>`
      : ""
  }
...
function disableLinkClicks() {
  document.querySelectorAll("a").forEach((link: HTMLElement) => {
    ...
    if (existingHandler) {
      originalLinkClickHandlers.set(
        link,
        existingHandler as (event: MouseEvent) => void,
      );
    }
...
export function highlightElement(element: Element, notes?: string[]) {
  ...
  if (!ALLOWED_HIGHLIGHT_TAGS.includes(finalTagName)) {
    ...
    if (
      ...
      ALLOWED_HIGHLIGHT_TAGS.includes(
        targetElement.parentElement.tagName.toUpperCase(),
      )
    ) {
  ...
  const isBlockElement =
    globalThis.getComputedStyle(targetElement).display === "block";
  addHighlight({
    xpath,
    content,
...
export function handleTextSelection(selection: Selection, notes?: string[]) {
  ...
  if (newHighlightDatas.length > 0) {
    ...
    for (const highlightData of newHighlightDatas) {
      ...
      currentBatchHighlights = mergeOverlappingHighlights(
        currentBatchHighlights,
        newHighlightWithNotes,
      );
    }
...
function getHighlightRanges(range: Range): TextHighlightData[] {
  ...
  const textNodeIterator = document.createNodeIterator(
    ...
    {
      acceptNode: (node) => {
        return range.intersectsNode(node) && node.nodeValue &&
  ...
  for (let i = 0; i < sortedBlocks.length; i++) {
    ...
    if (
      !blockElement.contains(startContainer) &&
    ...
    if (
      !blockElement.contains(endContainer) && !(blockElement === endContainer)
    ) {
    ...
    try {
      ...
      if (
        ...
        (blockElement.contains(
          currentBlockSelectionRange.commonAncestorContainer,
        ) ||
        ...
        const selectedTextContent = sanitizeAndPreserveFormatting(
          tempDivForBlock.innerHTML,
        );
...
export function sortHighlights() {
  highlights.sort((a, b) => {
    ...
    if (elementA && elementB) {
      const verticalDiff = getElementVerticalPosition(elementA) -
        getElementVerticalPosition(elementB);
      ...
      if (verticalDiff === 0) {
        ...
        return elementA.getBoundingClientRect().left -
          elementB.getBoundingClientRect().left;
      }
...
function areHighlightsAdjacent(
  highlight1: AnyHighlightData,
  highlight2: AnyHighlightData,
): boolean {
  if (
    highlight1.type === "text" && highlight2.type === "text" &&
    highlight1.xpath === highlight2.xpath
  ) {
...
function mergeOverlappingHighlights(
  ...
  for (const existing of existingHighlights) {
    if (
      doHighlightsOverlap(existing, newHighlight) ||
      areHighlightsAdjacent(existing, newHighlight)
    ) {
      if (!merged) {
...
function mergeHighlights(
  ...
  if (
    mergedElement !== element1 || mergedElement !== element2 ||
    highlight1.type === "complex" || highlight2.type === "complex"
  ) {
    return {
...
export function saveHighlights() {
  ...
  if (highlights.length > 0) {
    ...
    browser.storage.local.get("highlights").then(
      (result: { highlights?: HighlightsStorage }) => {
...
export async function loadHighlights() {
  ...
  if (
    storedData && Array.isArray(storedData.highlights) &&
    storedData.highlights.length > 0
  ) {
...
function isConsideredBlockElement(element: Element): boolean {
  ...
  return ALLOWED_HIGHLIGHT_TAGS.includes(tagName) &&
    BLOCK_LEVEL_TAGS_FOR_SPLIT.includes(tagName);
}
...
function getClosestAllowedBlock(node: Node | null): Element | null {
  ...
  while (current) {
    if (current.nodeType === Node.ELEMENT_NODE) {
      ...
      if (
        ALLOWED_HIGHLIGHT_TAGS.includes(el.tagName.toUpperCase()) &&
        isConsideredBlockElement(el)
      ) {
        return el;
      }
    }
