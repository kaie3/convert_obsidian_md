...
export class Reader {
  ...
  private static injectSettingsBar(doc: Document) {
    ...
    settingsBar.addEventListener("click", (e) => {
      ...
      switch (action) {
        case "decrease-font":
          this.updateFontSize(
            doc,
            parseInt(style.getPropertyValue("--obsidian-reader-font-size")) - 1,
          );
          break;
        case "increase-font":
          this.updateFontSize(
        ...
        case "decrease-width":
          this.updateWidth(
            doc,
            parseInt(style.getPropertyValue("--obsidian-reader-line-width")) -
              1,
          );
          break;
        case "increase-width":
        ...
        case "decrease-line-height":
          this.updateLineHeight(
            doc,
            parseFloat(
              style.getPropertyValue("--obsidian-reader-line-height"),
            ) - 0.1,
          );
          break;
    ...
    const themeSelect = settingsBar.querySelector(
    ...
    const themeModeSelect = settingsBar.querySelector(
  ...
  private static generateOutline(doc: Document) {
    ...
    headings.forEach((heading) => {
      ...
      if (!parentFound && lastHeadingAtLevel[level]) {
        const lastRect = lastHeadingAtLevel[level].element
      ...
      item.className =
        `obsidian-reader-outline-item obsidian-reader-outline-${heading.tagName.toLowerCase()}`;
      item.setAttribute("data-depth", depth.toString());
      item.textContent = heading.textContent;

      item.addEventListener("click", () => {
        const rect = heading.getBoundingClientRect();
        const scrollTop = globalThis.pageYOffset || doc.documentElement.scrollTop;
        const targetY = scrollTop + rect.top - globalThis.innerHeight * 0.05;
        globalThis.scrollTo({
          top: targetY,
          behavior: "smooth",
        });
  ...
  private static positionPopover(
    ...
    const spaceBelow = viewportHeight - linkRect.bottom - ARROW_HEIGHT -
      VIEWPORT_PADDING;
    const spaceAbove = linkRect.top - ARROW_HEIGHT - VIEWPORT_PADDING;
    const showBelow = spaceBelow >= popoverRect.height ||
    ...
    if (currentTop < VIEWPORT_PADDING) {
      ...
      const bottomOverflow = currentTop + popoverRect.height -
  ...
  private static cleanupScripts(doc: Document) {
    try {
      ...
      if (
        typeof window !== "undefined" && globalThis.clearTimeout &&
        globalThis.clearInterval
      ) {
        const nativeClearTimeout = globalThis.clearTimeout.bind(window);
        const nativeClearInterval = globalThis.clearInterval.bind(window);

        // Clear all timeouts and intervals
        let id = globalThis.setTimeout(() => {}, 0);
        while (id--) {
          try {
  ...
  static async apply(doc: Document) {
    try {
      ...
      const scripts = head.querySelectorAll(
        'script:not([type="application/ld+json"])',
      );
      scripts.forEach((el) => el.remove());
      ...
      doc.body.innerHTML = `
        ...
        [
          ...
          parseTime
            ? "parsed in " + new Intl.NumberFormat().format(parseTime) + " ms"
            : "",
        ].filter(Boolean).join(" · ")
      }
						</div>
      ...
      this.colorSchemeMediaQuery = globalThis.matchMedia(
        "(prefers-color-scheme: dark)",
      );
      this.colorSchemeMediaQuery.addEventListener(
        "change",
        (e) => this.handleColorSchemeChange(e, doc),
      );
  ...
  static restore(doc: Document) {
    if (this.originalHTML) {
      ...
      if (
        ...
        globalThis.hasOwnProperty("applyHighlights")
      ) {
