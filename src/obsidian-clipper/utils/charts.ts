...
export async function createUsageChart(
  container: HTMLElement,
  data: WeeklyUsage[],
): Promise<void> {
  // Calculate total clips for the period
  const totalClips = data[0].totalCount !== undefined
    ? data[0].totalCount
  ...
  const description = document.querySelector(
    ".usage-chart-title .setting-item-description",
  );
  if (description) {
    const message = totalClips === 1
      ? getMessage("pagesSaved")
      : getMessage("pagesSavedPlural");
  ...
  overlay.addEventListener("mousemove", (e) => {
    ...
    tooltip.innerHTML =
      `<div class="tooltip-date">${closestPoint.date}</div><div class="tooltip-count">${closestPoint.count}</div>`;
...
export function aggregateUsageData(
  ...
  if (options.timeRange === "all") {
    ...
    displayPeriods = options.aggregation === "day"
      ? 30
