export const remove_attr = (
  ...
  removeAttributes = removeAttributes.replace(/^(['"])(.*)\1$/, "$2").replace(
    /\\(['"])/g,
    "$1",
  );

  const removeAttributesList = removeAttributes.split(",").map((attr) =>
  ...
  return html.replace(
    /<(\w+)\s+([^>]*?)>/g,
    (match: string, tag: string, attributesString: string): string => {
      ...
      const allMatches = Array.from(
        attributesString.matchAll(attributeOrSlashRegex),
      );
      ...
      for (const currentMatch of allMatches) {
        ...
        if (attributePart) {
          ...
          if (nameMatch) {
            ...
            const shouldRemove = removeAttributesList.includes(
          ...
          slashPart && fullMatchedText && fullMatchedText.includes("/")
        ) {
