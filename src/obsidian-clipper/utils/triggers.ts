...
import { memoize, memoizeWithExpiration } from "./memoize.ts";
    ...
    for (const char of url) {
      ...
      if (node.templates.length > 0) {
        const matchingTemplate = node.templates.find((t) =>
          memoizedInternalMatchPattern(
            url.slice(0, url.indexOf(char) + 1),
            url,
            schemaOrgData,
          )
        );
...
const regexTriggers: Array<
  { template: Template; regex: RegExp; priority: number }
> = [];
const schemaTriggers: Array<
  { template: Template; pattern: string; priority: number }
> = [];
...
function matchSchemaPattern(pattern: string, schemaOrgData: any): boolean {
  const [, schemaType, schemaKey, expectedValue] =
    pattern.match(/schema:(@\w+)?(?:\.(.+?))?(?:=(.+))?$/) || [];
