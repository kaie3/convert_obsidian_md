...
export function handleDragStart(e: DragEvent): void {
  ...
  if (draggedElement && e.dataTransfer) {
    ...
    e.dataTransfer.setData(
      "text/plain",
      draggedElement.dataset.id || draggedElement.dataset.index || "",
    );
    setTimeout(() => {
...
export function handleDragOver(e: DragEvent): void {
  ...
  if (
    closestDraggable && closestDraggable !== draggedElement && draggedElement
  ) {
    const rect = closestDraggable.getBoundingClientRect();
    const midY = rect.top + rect.height / 2;
    if (e.clientY < midY) {
      closestDraggable.parentNode?.insertBefore(
        draggedElement,
        closestDraggable,
      );
    } else {
...
export function handleDrop(e: DragEvent): void {
  ...
  const list = (e.target as HTMLElement).closest(
    "ul, #template-properties, #model-list",
  );
...
function handleTemplateReorder(draggedItemId: string, newIndex: number): void {
  ...
  const oldIndex = templates.findIndex((t) =>
    (t as Template).id === draggedItemId
  );
...
export function handleTemplateDrag(): void {
  const templateSettingsForm = document.getElementById(
...
function handlePropertyReorder(draggedItemId: string, newIndex: number): void {
  ...
  const oldIndex = template.properties.findIndex((p) =>
    (p as Property).id === draggedItemId
  );
  if (oldIndex === -1) {
