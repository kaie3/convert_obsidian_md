...
export async function sendToLLM(
  ...
  if (now - lastRequestTime < RATE_LIMIT_RESET_TIME) {
    throw new Error(
      `Rate limit cooldown. Please wait ${
        Math.ceil((RATE_LIMIT_RESET_TIME - (now - lastRequestTime)) / 1000)
  ...
  try {
    ...
    Object.keys(parsedResponse.prompts_responses).forEach((key) => {
      if (typeof parsedResponse.prompts_responses[key] === "string") {
        parsedResponse.prompts_responses[key] = parsedResponse
          .prompts_responses[key]
          .replace(/\\n/g, "\n")
          .replace(/\r/g, "");
      }
    });
...
export async function initializeInterpreter(
  ...
  if (promptContextTextarea) {
    ...
    let promptToDisplay = template.context ||
  ...
  if (template) {
    ...
    if (interpretBtn && !generalSettings.interpreterAutoRun) {
      const clickListener = async () => {
        ...
        const modelConfig = generalSettings.models.find((m) =>
    ...
    if (modelSelect) {
      ...
      const enabledModels = generalSettings.models.filter((model) =>
        model.enabled
      );
      ...
      const lastSelectedModel = enabledModels.find((model) =>
        model.id === generalSettings.interpreterModel
      );
...
export async function handleInterpreterUI(
  ...
  try {
    ...
    const noteNameField = document.getElementById("note-name-field") as
      | HTMLTextAreaElement
      | null;
    ...
    interpreterErrorMessage.textContent = error instanceof Error
      ? error.message
...
export function replacePromptVariables(
  ...
  allInputs.forEach((input) => {
    if (
      ...
      if (
        input.id === "note-name-field" && input instanceof HTMLTextAreaElement
      ) {
