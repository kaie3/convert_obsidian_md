...
export function createMarkdownContent(content: string, url: string) {
  ...
  turndownService.addRule("listItem", {
    ...
    replacement: function (
      ...
      const checkbox = node.querySelector('input[type="checkbox"]') as
        | HTMLInputElement
        | null;
      let taskListMarker = "";
      ...
      if (parent instanceof HTMLOListElement) {
        ...
        let index = Array.from(parent.children).indexOf(node as HTMLElement) +
          1;
      ...
      if (figcaption) {
        ...
        captionContent = captionContent.replace(
          ...
          (match, mathContent, offset, string) => {
            ...
            const isEndOfLine = offset + match.length === string.length ||
              /\s/.test(nextChar);
  ...
  function extractLatex(element: Element): string {
    ...
    const mathNode = element.nodeName.toLowerCase() === "math"
      ? element
