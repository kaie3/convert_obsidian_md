...
} from "../managers/property-types-manager";
...
export async function exportTemplate(): Promise<void> {
  ...
  const orderedTemplate: Partial<Template> & { schemaVersion: string } = {
    ...
    properties: template.properties.map(({ name, value, type }) => ({
      ...
      value,
      type: type ||
        generalSettings.propertyTypes.find((pt) => pt.name === name)?.type ||
        "text",
    })),
...
export function importTemplate(input?: HTMLInputElement): void {
  ...
  const handleFile = (file: File) => {
    ...
    reader.onload = async (e: ProgressEvent<FileReader>) => {
      try {
        const importedTemplate = JSON.parse(
        ...
        importedTemplate.id = Date.now().toString() +
        ...
        if (importedTemplate.properties) {
          importedTemplate.properties = await Promise.all(
            importedTemplate.properties.map(async (prop: any) => {
              ...
              const type = generalSettings.propertyTypes.find((pt) =>
                pt.name === prop.name
              )?.type || "text";
        ...
        console.log(
          "Processed template properties:",
          importedTemplate.properties,
        );
...
function validateImportedTemplate(template: Partial<Template>): boolean {
  ...
  const hasValidNoteNameAndPath = isDailyNote ||
    (template.hasOwnProperty("noteNameFormat") &&
      template.hasOwnProperty("path"));

  // Add optional check for context
  const hasValidContext = !template.context ||
...
async function processImportedTemplate(
  importedTemplate: Partial<Template>,
): Promise<Template> {
  ...
  if (importedTemplate.properties) {
    ...
    for (const prop of importedTemplate.properties) {
      console.log(
        `Processing property: ${prop.name}, type: ${
          prop.type || "text"
        }, value: ${prop.value}`,
      );
    ...
    importedTemplate.properties = importedTemplate.properties.map((prop) => {
      ...
      return {
        ...
        name: prop.name,
        value: prop.value,
        type: existingPropertyType
          ? existingPropertyType.type
          : (prop.type || "text"),
      };
    });
  }
...
async function importTemplateFromJson(jsonContent: string): Promise<void> {
  try {
    ...
    console.error("Error parsing imported template:", error);
    throw new Error(
      "Error importing template. Please check the file and try again.",
    );
  }
}
...
async function importAllSettingsFromJson(jsonContent: string): Promise<void> {
  try {
    ...
    throw new Error(
      "Error importing settings. Please check the file and try again.",
    );
  }
}
