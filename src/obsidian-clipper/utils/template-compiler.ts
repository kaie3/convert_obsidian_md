import { processSimpleVariable } from "./variables/simple.ts";
import { processSelector } from "./variables/selector.ts";
import { processSchema } from "./variables/schema.ts";
import { processPrompt } from "./variables/prompt.ts";

import { processForLoop } from "./tags/for.ts";
...
export async function processLogic(
  ...
  for (const handler of logicHandlers) {
    ...
    while ((match = handler.regex.exec(processedText)) !== null) {
      ...
      processedText = processedText.substring(0, match.index) + result +
...
export async function processVariables(
  ...
  while ((match = regex.exec(result)) !== null) {
    ...
    if (
      trimmedMatch.startsWith("selector:") ||
      trimmedMatch.startsWith("selectorHtml:")
    ) {
      ...
      trimmedMatch.startsWith('"') || trimmedMatch.startsWith("prompt:")
    ) {
      replacement = await processPrompt(fullMatch, variables, currentUrl);
    } else {
      replacement = await processSimpleVariable(
        trimmedMatch,
        variables,
        currentUrl,
      );
