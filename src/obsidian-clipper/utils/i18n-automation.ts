...
export default class I18nAutomation {
  ...
  private chatHistories: {
    [locale: string]: { role: string; content: string }[];
  } = {};
  ...
  private initializeChatHistory(targetLanguage: string) {
    this.chatHistories[targetLanguage] = [{
      role: "system",
      content:
        `You are a professional translator for the Obsidian Web Clipper browser extension.
...
{"save":"Sauvegarder dans le coffre","error":"Échec de la connexion"}`,
  ...
  private async translateBatch(
    ...
    this.chatHistories[targetLanguage].push({
      role: "user",
      content:
        `Translate these messages to ${targetLanguage}. Respond with a valid JSON object where keys match the input keys and values are the translations. Format the response as a single line without pretty-printing:\n\n${batchPrompt}`,
    });
    ...
    try {
      ...
      Object.entries(translations).forEach(([key, translation]) => {
        console.log(
          `    - ${key}: "${translation.substring(0, 40)}${
  ...
  async processLocales(srcDir: string, targetLocale?: string): Promise<void> {
    ...
    const sourceMessages: Messages = JSON.parse(
      await fs.promises.readFile(sourceFile, "utf-8"),
    );
    console.log(
      `✓ Found ${Object.keys(sourceMessages).length} source messages`,
    );
    ...
    const localesToProcess = targetLocale
      ...
      : locales.filter((locale) =>
        !locale.startsWith(".") && locale !== this.sourceLocale
      );
    ...
    for (const locale of localesToProcess) {
      ...
      if (!locales.includes(locale)) {
        ...
        await fs.promises.mkdir(path.join(this.localesDir, locale), {
          recursive: true,
        });
      }
      ...
      try {
        localeMessages = JSON.parse(
          await fs.promises.readFile(localeFile, "utf-8"),
        );
        console.log(`  📂 Found existing translations for ${locale}`);
      } catch (error) {
        console.log(
          `  ⚠️  No existing translations found for ${locale}, creating new file`,
        );
      }

      // Find missing translations
      const missingKeys = Object.keys(sortedSourceMessages).filter((key) =>
        !localeMessages[key]
      );
      if (missingKeys.length > 0) {
        ...
        for (let i = 0; i < missingKeys.length; i += this.batchSize) {
          ...
          for (let attempt = 1; attempt <= 2; attempt++) {
            try {
              ...
              console.error(
                `  ❌ Both translation attempts failed, using source messages as fallback`,
              );
      ...
      const obsoleteKeys = Object.keys(localeMessages).filter((key) =>
        !sortedSourceMessages[key]
      );
