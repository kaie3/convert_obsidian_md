import browser from "./browser-polyfill.ts";
import { escapeDoubleQuotes, sanitizeFileName } from "./string-utils.ts";
import { Property, Template } from "../types/types.ts";
...
export function saveToObsidian(
  ...
  if (isDailyNote) {
    ...
    obsidianUrl = `obsidian://new?file=${
      encodeURIComponent(path + formattedNoteName)
    }`;
  }
