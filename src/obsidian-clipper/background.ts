...
import { ensureContentScriptLoaded } from "./utils/content-script-utils.ts";
import { detectBrowser } from "./utils/browser-detection.ts";
import {
  isBlankPage,
  isValidUrl,
  updateCurrentActiveTab,
} from "./utils/active-tab-manager.ts";
import { TextHighlightData } from "./utils/highlighter.ts";
...
async function initialize() {
  try {
    ...
    const result = await browser.storage.local.get("isHighlighterMode") as {
...
browser.runtime.onMessage.addListener(
  (
    ...
    if (typeof request === "object" && request !== null) {
      ...
      if (
        typedRequest.action === "extractContent" && sender.tab && sender.tab.id
      ) {
      ...
      if (
        typedRequest.action === "highlighterModeChanged" && sender.tab &&
      ...
      if (
        typedRequest.action === "updateHasHighlights" && sender.tab &&
      ...
      if (
        typedRequest.action === "toggleHighlighterMode" && typedRequest.tabId
      ) {
      ...
      if (tabs[0]?.id) {
        ...
        setTimeout(() => {
          browser.runtime.sendMessage({ action: "triggerQuickClip" })
            .catch((error) =>
              console.error("Failed to send quick clip message:", error)
            );
        }, 500);
      }
    });
...
browser.contextMenus.onClicked.addListener(async (info, tab) => {
  if (info.menuItemId === "open-obsidian-clipper") {
    ...
    info.menuItemId === "open-side-panel" && tab && tab.id && tab.windowId
  ) {
